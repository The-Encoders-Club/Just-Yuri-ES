init python:
    ...
    def invert():
        ...
        inv_surf = inv.canvas().get_surface()  
        inv_surf.fill((255, 255, 255, 255))
        inv_surf.blit(srf, (0, 0), None, 2) 
        return inv_surf  
...
init python:
    ...
    class Tear(renpy.Displayable):
        def __init__(self, number, offtimeMult, ontimeMult, offsetMin, offsetMax, srf=None):
            ...
            if not srf: self.srf = screenshot_srf(False) 
            else: self.srf = srf
...
init python:
    ...
    class RectStatic(object):
        def __init__(self, theDisplayable, numRects=12, rectWidth = 30, rectHeight = 30):
            self.sm = renpy.display.particle.SpriteManager(update=self.update) 
            self.rects = [ ]
            self.timers = [ ]
